...
class UserEnumLabel(QWidget):
    def __init__(self, label_text="", param_full_path="",label_width=150, parent=None):
        ...
        if param_full_path:
            ...
            if self.param:
                ...
                self.handle_value_changed(self.param.value)
                self.handle_is_err_changed(self.param.is_err)
                self.handle_is_not_support_changed(self.param.is_not_support)
    ...
    def handle_value_changed(self, value : str):
        if self.param:
            self.param.ref_list : Optional[Type[DescriptionEnum]] 
            n_value = int(value)
            if self.param.ref_list is not None:
                self.setText(self.param.ref_list(n_value).description)
            else:
                self.setText("Unknown Value")
            
    def handle_is_err_changed(self, is_err : bool):
        if is_err:

...
class PosiInputInMain(QGroupBox):
    BORDER_COLOR = "#dcdcdc"
    HOVER_COLOR  = "#1a73e8"
    ...
    def __init__(self, title_text="", param_full_path = "", parent=None):
        ...
        self.setStyleSheet(f"""
            PosiInputInMain {{ 
                ...
                font-weight: normal; 
                color: black; 
                border: 1px solid {PosiInputInMain.BORDER_COLOR}; 
                margin-top: 10px; 
            }}
            PosiInputInMain[my_hover="true"] {{
                border: 1px solid {PosiInputInMain.HOVER_COLOR};
            }}
        """)
        ...
        self.line_edit.setAttribute(Qt.WidgetAttribute.WA_Hover, True) 
    ...
    def eventFilter(self, obj, event):
        if obj == self.line_edit:
            if event.type() == QEvent.Type.HoverEnter:
                self.setProperty("my_hover", "true")
                self.style().unpolish(self)
                self.style().polish(self)
                
            # 💡 [추가] 자식 위젯에서 마우스가 나갈 때 부모 테두리 스타일 복구
            elif event.type() == QEvent.Type.HoverLeave:
                self.setProperty("my_hover", "false")
                self.style().unpolish(self)
                self.style().polish(self)
    ...
    def handle_value_changed(self):
        if self.param:
            if not self.param.is_not_support:
                ...
                if value:
                    display_value = self.converter.convert_posi_to_display_value(value)
    ...
    def getParamWriteValue(self) -> str:
        param_value_str = self.converter.convert_display_to_posi_value_str(self.value())

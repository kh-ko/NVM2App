...
from c_ui.b_control_packet.controls_with_label.l_bitmap_ro_v_widget import LBitmapReadOnlyVerticalWidget


class ParamBitmapReadOnlyWidget(LBitmapReadOnlyVerticalWidget):
    def __init__(self, param_full_path : str, parent = None):
        self.param = ParamManager().get_by_full_path(param_full_path)

        super().__init__(label_text=self.param.name, enum_class=self.param.ref_list, parent = parent)
